...
export default function ProfileTab({ user, newPlateInput, setNewPlateInput, handleAddPlate, handleDeletePlate, setActiveTab, loadUserData }) {
  ...
  const addPlateToDriverProfile = async (plate) => {
    /*
    * NOTE Quảng - Driver scope:
    * Khi Driver mua vé bằng biển số mới, FE tự thêm biển số đó vào hồ sơ Driver trước.
    * Không sửa parkingApi.js để tránh đụng file shared của team.
    */
    const token =
    ...
    if (!response.ok || result.success === false) {
      const message = result.message || "Không thể thêm biển số mới vào hồ sơ Driver";

      /*
      * Nếu backend báo biển số đã tồn tại, vẫn cho đi tiếp.
      * Có thể biển số đã được thêm trước đó nhưng local user chưa sync lại.
      */
      if (
  ...
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/driver/parking-passes/${passId}/cancel`,
      {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    if (!response.ok || result.success === false) {
      throw new Error(result.message || "Không thể hủy đơn thanh toán");
    }
