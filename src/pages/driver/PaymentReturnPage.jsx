...
function ReceiptRow({ label, value, highlight, mono }) {
  return (
    <div className="flex items-center justify-between py-2.5 border-b border-slate-100 last:border-0">
      ...
      <span
        className={`text-sm font-bold ${highlight ? "text-emerald-600" : "text-slate-800"} ${mono ? "font-mono tracking-wider" : ""}`}
      >
        {value}
      </span>
    </div>
...
export default function PaymentReturnPage() {
  ...
  const isSessionPayment =
    state.data?.paymentType === "SESSION" || txnRef.startsWith("SESSION-");
  const amount = params.get("vnp_Amount")
    ? (parseInt(params.get("vnp_Amount")) / 100).toLocaleString("vi-VN") + "đ"
    : "—";
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-cyan-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-6 text-sm font-bold text-slate-500 animate-pulse">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-cyan-50 relative overflow-hidden">
      ...
      <div
        className={`relative z-10 flex min-h-screen flex-col items-center justify-center px-5 py-10 transition-all duration-700 ${showContent ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
      >
        {/* main card */}
        <div className="w-full max-w-lg">
          ...
          <div className="text-center mb-2">
            <span
              className={`inline-flex items-center gap-1.5 rounded-full px-4 py-1.5 text-[10px] font-black uppercase tracking-[0.2em] ${
                state.success
            ...
            >
              <span
                className={`h-1.5 w-1.5 rounded-full ${state.success ? "bg-emerald-500" : "bg-red-500"} animate-pulse`}
              />
          ...
          <div className="mt-8 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl shadow-slate-200/50">
            {/* receipt header */}
            <div
              className={`px-6 py-4 ${state.success ? "bg-gradient-to-r from-emerald-500 to-teal-500" : "bg-gradient-to-r from-red-500 to-rose-500"}`}
            >
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:justify-center">
            <button
              onClick={() =>
                navigate(
                  isSessionPayment
                    ? "/driver/dashboard#history-table"
                    : "/driver/dashboard#profile-vip",
                )
              }
          ...
          <div className="mt-6 flex items-start gap-3 rounded-2xl border border-slate-100 bg-white/60 backdrop-blur-sm p-4">
            ...
            <p className="text-[11px] leading-5 text-slate-400">
              ...
              <span className="font-bold text-slate-500">VNPay</span> với mã hóa
              SSL 256-bit. Mọi thông tin thẻ không được lưu trữ trên hệ thống
              Smart Parking.
            </p>
          </div>
        </div>
