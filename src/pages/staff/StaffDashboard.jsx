  ...
  useEffect(() => {
    const loadStaffDashboard = async () => {
      try {
        ...
        setApiError("Không lấy được dữ liệu từ API.");
  ...
  if (!dashboardData) {
    return (
      <div className="p-8 text-red-600">
        {apiError || "Không lấy được dữ liệu từ API."}
      </div>
    );
  }
        ...
        <section className="space-y-8 p-8">
          <div className="rounded-2xl bg-slate-950 p-8 text-white">
            <h1 className="text-3xl font-bold">
              Xin chào, {dashboardData.staffName || "Staff"} 👋
            </h1>

            <p className="mt-2 text-slate-300">
              Theo dõi tình trạng bãi xe, hỗ trợ check-in/check-out và xử lý các
              phiên gửi xe trong ngày.
            </p>
          </div>

          <div className="grid grid-cols-1 gap-5 md:grid-cols-3 xl:grid-cols-6">
            <StatCard title="Chỗ trống" value={stats.availableSlots || 0} />
            <StatCard title="Đang có xe" value={stats.occupiedSlots || 0} />
            <StatCard title="Đã đặt" value={stats.reservedSlots || 0} />
            <StatCard title="Xe vào hôm nay" value={stats.todayCheckIn || 0} />
            <StatCard title="Xe ra hôm nay" value={stats.todayCheckOut || 0} />
            <StatCard title="Doanh thu hôm nay" value={stats.revenue || "0đ"} />
          </div>
