...
export default function StaffZoneExit() {
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      try {
        ...
        // Lấy các cổng Zone đầu ra (ZONE_EXIT hoặc ZONE_BOTH)
        const zoneGates = (config.gates || [])
          .filter(g => g.gateType === 'ZONE_EXIT')
          .map(g => {
        ...
        try {
          ...
          await scanner.start(
            ...
            (decodedText) => {
              ...
              if (handleZoneCheckOutRef.current) {
                handleZoneCheckOutRef.current(decodedText);
              }
            },
            (errorMessage) => {
  ...
  // Xử lý quét xác nhận
  const handleZoneCheckOut = async (codeToSubmit = ticketInput) => {
    ...
    if (!selectedGateId) {
      setApiError("Vui lòng chọn cổng ra Zone!");
      return;
    }
    ...
    try {
      const res = await staffApi.zoneExit({
        sessionCode: code,
      ...
      setApiSuccess(`Check-out ra khỏi ${data.zoneName} thành công. Barie mở.`);
    } catch (err) {
      console.error("Zone exit error:", err);
      setBarrierState("CLOSED");
      const msg = err.response?.data?.message || err.message || "Lỗi xử lý check-out phụ.";
      setApiError(msg);
    } finally {
  ...
  const handleMockScan = async () => {
    try {
      ...
      // Tìm session đang ACTIVE mà đã vào zone nhưng chưa ra zone
      const activeInZone = allSessions.find(s => s.status === "ACTIVE" && s.entryZoneGate && !s.exitZoneGate);

      if (activeInZone) {
        setTicketInput(activeInZone.sessionCode);
        handleZoneCheckOut(activeInZone.sessionCode);
      } else {
        // Fallback: Lấy session active bất kỳ
        const activeAny = allSessions.find(s => s.status === "ACTIVE");
        if (activeAny) {
          setTicketInput(activeAny.sessionCode);
          handleZoneCheckOut(activeAny.sessionCode);
        } else {
          setApiError("Không tìm thấy phiên gửi xe nào đang hoạt động trong bãi! Hãy tạo một Check-in lần 1 và vào Zone trước.");
        }
      }
    } catch (err) {
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
        ...
        <div className="space-y-4 lg:col-span-9 flex flex-col">
            ...
            <div className="grid grid-cols-2 gap-[2px] p-[2px]">
              ...
              <div className="flex flex-col gap-[2px] h-full justify-between">
                ...
                <div className="relative bg-slate-700 aspect-[4/3] flex items-center justify-center overflow-hidden shrink-0">
                  ...
                  <span className="absolute top-2 left-2 text-white font-black text-[9px] px-1.5 py-0.5 rounded tracking-wider bg-slate-900/60">
                    CAM 1: QUÉT QR VÉ RA ZONE
                  </span>
                </div>
                ...
                <div className="bg-slate-50 p-2 border-t border-slate-200 flex flex-col gap-1 mt-auto h-[82px] min-h-[82px] justify-center overflow-y-auto">
                  {scanResult ? (
                    <div className="text-[10px] space-y-0.5">
                      ...
                      <div className="flex justify-between">
                        <span className="text-slate-400 font-bold uppercase">Khu đỗ vừa ra:</span>
                        <span className="font-black text-rose-600 uppercase">{scanResult.zoneName} ({scanResult.floorName})</span>
                      </div>
                    </div>
                  ) : (
          ...
          <div className="rounded-2xl border border-slate-200 bg-white shadow-sm p-4 space-y-3 max-w-[1000px] w-full">
            <div>
              <label className="block text-[10px] font-black uppercase tracking-wider text-slate-400 mb-1.5">Cổng phụ ra (Zone Exit Gate)</label>
              {configLoaded ? (
                <select
        ...
        <div className="space-y-3 lg:col-span-3 flex flex-col">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-4 space-y-2.5 shadow-sm">
            ...
            <p className="text-[9px] text-slate-450 leading-relaxed font-semibold">
              (Nhấn để tự động quét xe đang đỗ trong Zone đi ra ngoài)
            </p>
          </div>
          ...
          {scanResult && (
            <div className="rounded-xl border border-slate-200 bg-white p-4 space-y-3 shadow-sm animate-fade-in">
              ...
              <div className="space-y-2 text-xs">
                ...
                <div>
                  <span className="text-slate-400 font-bold block text-[9px] uppercase">Khu đỗ vừa ra:</span>
                  <span className="font-black text-rose-600 block">{scanResult.zoneName} ({scanResult.floorName})</span>
                </div>
