...
export default function StaffMapping({ onLogout }) {
  ...
  const floorSlots = useMemo(() => {
    return groups.flatMap((group) =>
      group.zones.flatMap((zone) => createMockSlots(zone))
    );
  }, [groups]);
  ...
  const filterSlot = (slot) => {
    ...
    const matchSearch =
      ...
      (slot.plate && slot.plate.toLowerCase().includes(keyword));
            ...
            <section key={group.category} className="space-y-6">
              ...
              <div className="grid grid-cols-1 gap-10 xl:grid-cols-2">
                {group.zones.map((zone) => {
                  const slots = createMockSlots(zone).filter(filterSlot);

                  return (
                    <div key={zone.prefix} className="space-y-4">
                      <div className="flex items-center gap-3">
                        <h3 className="text-xs font-bold uppercase tracking-widest text-slate-500">
                          {zone.section} ({slots.length}/{zone.slots} ô)
                        </h3>
          ...
          {isAvailable ? (
            <>
              ...
              <p className="text-sm text-slate-500">
                Nhân viên có thể giữ chỗ hoặc hướng dẫn xe vào vị trí này.
              </p>
            </>
          ) : (
            <>
