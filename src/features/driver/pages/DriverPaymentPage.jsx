...
export default function DriverPaymentPage({ onLogout }) {
  const [form, setForm] = useState({ licensePlate: "", amount: 0, method: "BANK_TRANSFER", reason: "Thanh toán lượt gửi xe" });
  const [payments, setPayments] = useState([]);
  const [latest, setLatest] = useState(null);

  useEffect(() => {
    let mounted = true;
    getPayments().then((data) => mounted && setPayments(data));
  ...
  return (
    <DriverShell title="Thanh toán" subtitle="Tạo QR thanh toán hoặc QR xác nhận đã thanh toán khi ra bãi" onLogout={onLogout}>
      <div className="grid gap-6 xl:grid-cols-[1fr_0.75fr]">
        <Panel title="Tạo giao dịch thanh toán" subtitle="Gọi API tạo giao dịch tiền mặt hoặc chuyển khoản">
