...
export default function DriverPassBuyPage({ onLogout }) {
  const [plans, setPlans] = useState([]);
  const [form, setForm] = useState({ licensePlate: "", vehicleType: "MOTORBIKE", paymentMethod: "BANK_TRANSFER" });
  ...
  useEffect(() => {
    ...
    async function loadData() {
      const [nextPlans, nextPasses] = await Promise.all([getMembershipPlans(), getPasses()]);
      if (!mounted) return;
      setPlans(nextPlans);
  ...
  async function buy(planId) {
    const pass = await createPass({ ...form, planId });
  ...
  return (
    <DriverShell title="Gói thành viên" subtitle="Mua gói gửi xe tháng, quý, năm và nhận QR vé thành viên" onLogout={onLogout}>
      <div className="space-y-6">
        ...
        <Panel title="Vé thành viên đã mua" subtitle="Dữ liệu lấy từ API">
