...
const initialForm = { licensePlate: "59A1-123.45", vehicleType: "MOTORBIKE", floorId: "A2", zone: "A", startAt: "" };
...
export default function DriverReservation({ onLogout }) {
  ...
  function submit(event) {
    ...
    const reservation = createReservation({ ...form, floorName: selectedFloor?.name || form.floorId });
  ...
  return (
    <DriverShell title="Đặt chỗ trước" subtitle="Chọn tầng, khu A/B/C/D và nhận mã QR để Staff xác nhận" onLogout={onLogout}>
      <div className="grid gap-6 xl:grid-cols-[1fr_0.8fr]">
        <Panel title="Thông tin đặt chỗ" subtitle="QR xuất hiện sau khi đặt chỗ thành công">
          <form onSubmit={submit} className="grid gap-4 md:grid-cols-2">
            ...
            <Field label="Khu trong tầng"><select value={form.zone} onChange={(e) => update("zone", e.target.value)} className="input-pro">{(selectedFloor?.zones || ["A", "B", "C", "D"]).map((zone) => <option key={zone} value={zone}>Khu {zone}</option>)}</select></Field>
      ...
      <Panel title="Danh sách đặt chỗ của tôi" subtitle="Có thể hủy đặt chỗ demo nếu chưa check-in" className="mt-6">
