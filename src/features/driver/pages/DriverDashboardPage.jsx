...
export default function DriverDashboardPage({ onLogout }) {
  const [analytics, setAnalytics] = useState({ floors: [], totalAvailable: 0, totalCapacity: 0, totalRevenue: 0 });
  ...
  useEffect(() => {
    ...
    async function loadDashboard() {
      const [nextAnalytics, nextReservations, nextPasses] = await Promise.all([
        getAnalytics(),
        getReservations(),
        getPasses(),
      ]);
      ...
      setReservations(nextReservations.filter((item) => item.status === "CONFIRMED"));
      setPasses(nextPasses.filter((item) => item.status === "ACTIVE"));
    }
    loadDashboard();
    ...
    <DriverShell title="Driver Dashboard" subtitle="Đặt chỗ, mua gói thành viên và theo dõi xe của bạn" onLogout={onLogout}>
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <StatCard icon="₫" label="Đã thanh toán" value={formatVnd(analytics.totalRevenue)} hint="Dữ liệu từ API" />
        </div>
