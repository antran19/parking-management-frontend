...
export default function DriverProfilePage({ onLogout }) {
  const [vehicles, setVehicles] = useState([]);

  useEffect(() => {
    let mounted = true;
    getVehicles().then((data) => mounted && setVehicles(data));
  ...
  return (
    <DriverShell title="Hồ sơ xe" subtitle="Quản lý thông tin cá nhân và danh sách xe của Driver" onLogout={onLogout}>
      <div className="grid gap-6 xl:grid-cols-[0.8fr_1.2fr]">
        <Panel title="Thông tin Driver" subtitle="Thông tin tài khoản lấy từ API đăng nhập">
          <div className="space-y-4">
            <Info label="Họ tên" value="Driver" />
            <Info label="Email" value="Đọc từ API auth/profile" />
            <Info label="Số điện thoại" value="Đọc từ API auth/profile" />
