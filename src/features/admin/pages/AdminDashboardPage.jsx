...
export default function AdminDashboardPage({ onLogout }) {
  const [analytics, setAnalytics] = useState({ floors: [], employees: [], drivers: [], totalCapacity: 0, totalOccupied: 0, totalReserved: 0, totalAvailable: 0, totalRevenue: 0 });
  const [activity, setActivity] = useState([]);

  useEffect(() => {
    let mounted = true;
    async function loadDashboard() {
      const [nextAnalytics, nextActivity] = await Promise.all([getAnalytics(), getActivity()]);
      if (!mounted) return;
    ...
    <AdminShell title="Admin Dashboard"  onLogout={onLogout}>
      <div className="space-y-6">
          ...
          <div className="grid gap-6 lg:grid-cols-[1.25fr_0.75fr] lg:items-center">
            <div>
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                ...
                <Link to="/admin/mapping" className="rounded-2xl bg-white/10 px-5 py-3 text-sm font-black text-white ring-1 ring-white/20">Quản lý tầng/khu</Link>
              </div>
            </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <StatCard icon="₫" label="Doanh thu" value={formatVnd(analytics.totalRevenue)} hint="Payment + History API" />
        </div>
