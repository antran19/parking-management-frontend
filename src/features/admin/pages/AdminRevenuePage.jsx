...
export default function AdminRevenuePage({ onLogout }) {
  const [a, setAnalytics] = useState({ payments: [], history: [], totalRevenue: 0 });

  useEffect(() => {
    let mounted = true;
    getAnalytics().then((data) => mounted && setAnalytics(data));
  ...
  return <AdminShell title="Thanh toán & doanh thu" subtitle="Admin theo dõi giao dịch, doanh thu gửi xe và thanh toán QR" onLogout={onLogout}>
    ...
    <Panel title="Biểu đồ doanh thu" className="mt-6"><div className="space-y-3">{rows.map(r=><div key={r.id} className="grid grid-cols-[150px_1fr_140px] items-center gap-3 text-sm font-bold"><span>{r.plate}</span><div className="h-4 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-[#001e40]" style={{width:`${Math.max(8,(Number(r.amount)||0)/max*100)}%`}} /></div><span className="text-right font-black">{formatVnd(r.amount)}</span></div>)}</div></Panel>
