...
export default function AdminStaffPage({ onLogout }) {
  const [employees, setEmployees] = useState([]);

  useEffect(() => {
    let mounted = true;
    getEmployees().then((data) => mounted && setEmployees(data));
  ...
  async function submit(event) {
    ...
    if (editingId) {
      ...
      await addActivity(`Admin sửa nhân viên ${form.fullName}`);
    } else {
      next = [{ id: uid("EMP"), ...form, createdAt: new Date().toISOString().slice(0, 10) }, ...employees];
      await addActivity(`Admin thêm nhân viên ${form.fullName}`);
    }
