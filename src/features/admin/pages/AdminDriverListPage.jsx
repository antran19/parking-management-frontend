...
export default function AdminDriverListPage({ onLogout }) {
  const [drivers, setDrivers] = useState([]);

  useEffect(() => {
    let mounted = true;
    getDrivers().then((data) => mounted && setDrivers(data));
  ...
  async function submit(e){ e.preventDefault(); const next=[{id:uid("DRV"),...form,licensePlate:form.licensePlate.toUpperCase()},...drivers]; await saveDrivers(next); setDrivers(next); setForm(empty); }
  async function toggle(id){ const next=drivers.map((d)=>d.id===id?{...d,status:d.status==="ACTIVE"?"LOCKED":"ACTIVE"}:d); await saveDrivers(next); setDrivers(next); }
  async function remove(id){ const next=drivers.filter((d)=>d.id!==id); await saveDrivers(next); setDrivers(next); }
