...
export default function AdminPackagePlanPage({ onLogout }) {
  const [plans, setPlans] = useState([]);

  useEffect(() => {
    let mounted = true;
    getMembershipPlans().then((data) => mounted && setPlans(data));
    return () => { mounted = false; };
  }, []);

  async function update(id, field, value){ const next=plans.map(p=>p.id===id?{...p,[field]:field==='price'||field==='months'?Number(value):value}:p); await saveMembershipPlans(next); setPlans(next); }
