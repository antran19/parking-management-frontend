...
import { getFloors, getFloorUsage, updateFloorCapacity } from "../../../shared/services/smartParkingStore";
import AdminShell from "../AdminShell";

export default function AdminMappingPage({ onLogout }) {
  const [floors, setFloors] = useState([]);

  useEffect(() => {
    let mounted = true;
    getFloors().then((data) => mounted && setFloors(data));
    return () => { mounted = false; };
  }, []);

  async function change(id, field, value) {
    const nextFloors = await updateFloorCapacity(id, { [field]: Number(value) });
    setFloors(nextFloors);
  }

  return <AdminShell title="Admin Mapping" subtitle="Quản lý tầng, khu và sức chứa bằng API" onLogout={onLogout}>
